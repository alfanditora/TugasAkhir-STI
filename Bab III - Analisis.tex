...
\begin{enumerate}
  ...
  Berbeda dengan foto kueri yang berupa selfie frontal (menghadap ke depan), foto target dalam basis data sering kali diambil secara kandid (candid), miring (non-frontal), atau dengan ekspresi wajah yang ekstrem. Algoritma yang tidak memiliki fitur \textit{face alignment} yang kuat akan menghasilkan jarak Euclidean atau kosinus yang jauh, sehingga menyebabkan tingginya angka penolakan palsu (\textit{False Reject Rate}).
  ...
  Citra digital dunia nyata sering kali memiliki resolusi sangat tinggi ($\ge 8\text{ MP}$ atau resolusi $4\text{K}$). Proses inferensi menggunakan detektor wajah standar pada resolusi asli ini menyebabkan lonjakan drastis pada komputasi grafis dan memicu Out of Memory (OOM) pada spesifikasi perangkat keras standar. Oleh karena itu, diperlukan mekanisme Progressive Resolution Fallback Cascade untuk mereduksi skala citra secara adaptif ke resolusi maksimal $1080\text{p}$ ($1920 \times 1080$ piksel) sebelum proses deteksi dilakukan.
...
\begin{enumerate}
  \item \textbf{Tingginya Kompleksitas Komputasi Deep Learning} \\
  Algoritma modern berbasis Convolutional Neural Network (CNN) dalam fase ekstraksi fitur (\textit{face representation}) membutuhkan operasi \textit{floating-point} yang sangat masif. Jika sistem dijalankan pada spesifikasi perangkat keras standar tanpa bantuan \textit{Graphics Processing Unit} (GPU) eksternal, waktu pemrosesan rata-rata per foto berisiko menjadi lebih lambat.

  \item \textbf{Tantangan Skalabilitas Memori (Memory Footprint)} \\
